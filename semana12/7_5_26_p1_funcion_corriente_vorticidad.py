...
def relax():
    ...
    for i in range(1, Nx -1):
        for j in range(1, Ny -1):
            ...
            max_residual = max(max_residual, np.abs(ru), np.abs(rw))
...
for it in range(max_iter):
    ...
    residual = relax()
    if it % 100 == 0:
        print(f"Iteration {it}, residual = {residual}")
        print(f'u upstream = {u[x0 - 5, Ny // 2]}')
        print(f'u arriba = {u[x0 + L_beam // 2, y1 + 2]}')
        print(f'u downstream = {u[x1 + 5, Ny // 2]}')
...
# grafica de funcion de corriente
X, Y = np.meshgrid(np.arange(Nx), np.arange(Ny))
plt.figure(figsize = (10, 5))
plt.contourf(X, Y, u.T, levels = 20)
...
# lineas de corriente
plt.figure(figsize = (10, 5))
plt.streamplot(X, Y, vx.T, vy.T, density=1.5)
